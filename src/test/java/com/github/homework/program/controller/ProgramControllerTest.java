...
public class ProgramControllerTest extends BaseControllerTest {
    ...
    public void getProgramTest() throws Exception {
        Program program = givenProgram(givenTheme());
        this.mockMvc.perform(get("/api/programs/{id}", program.getId()))
            ...
            .andExpect(jsonPath("introductionDetail")
                .value("여행자와 현지인이 꼽은 최고의 먹거리 여행지' 에서 대한민국 229개 지방자치단체 중 여수시가 1위에 선정되어 식도락 여행에 최적화된 프로그램"))
        ...
        this.mockMvc.perform(get("/api/programs")
            ...
            .andDo(write.document(
                requestParameters(
                    ...
                    parameterWithName("size").description("Content size")
                )
            ));
        ;
    }
    ...
    private Theme givenTheme() {
        return this.themeRepository.save(new Theme("식도락여행"));
    }
}